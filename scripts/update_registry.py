...
def _estimate_ram(name: str) -> tuple[float, float]:
    """Rough RAM estimate from model name. Returns (min_ram_gb, min_disk_gb).
    
    Based on rule of thumb: Q4 quantization needs ~0.6 GB per billion params.
    We parse param count from the name when possible.
    """
    import re
    name_lower = name.lower()

    # look for patterns like 7b, 13b, 70b, 671b, 1.5b, 0.5b
    match = re.search(r"(\d+\.?\d*)b", name_lower)
    if match:
        params_b = float(match.group(1))
        # Q4 quant = ~0.6 GB/B params, add 1GB overhead
        min_ram = round(params_b * 0.6 + 1, 1)
        disk = round(params_b * 0.55, 1)
        return max(min_ram, 1.0), max(disk, 0.5)

    # couldn't parse — conservative default
    return 4.0, 2.0


def _needs_avx2(name: str) -> bool:
    # models over ~7B generally need AVX2 for decent performance
    import re
...
def hf_to_registry_entry(hf_model: dict) -> dict | None:
    ...
    if any(x in name_lower for x in ["code", "coder", "coding", "starcoder", "deepseek-coder"]):
        ...
        tags = ["embedding"]
    elif any(x in name_lower for x in ["vision", "llava", "moondream", "visual", "vl"]):
        tags = ["vision"]
